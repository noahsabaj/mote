"""Generation engine: prefill + byte-at-a-time decoding, emitting the telemetry events described in
docs/api.md. Synchronous; run it in a worker thread.
"""
...
STOP_IDS = {EOS_ID, PAD_ID, SYSTEM_ID, USER_ID, ASSISTANT_ID, BOS_ID, RESULT_ID,
            FIM_PREFIX_ID, FIM_SUFFIX_ID, FIM_MIDDLE_ID, R2L_ID, OFFSET_ID}
...
def _dist(logits: torch.Tensor, temperature: float, top_p: float) -> torch.Tensor:
    ...
    temperature <= 0 is greedy (a one-hot). The host reference the graph decoder's device sampler is
    tested against (tests/test_graph_decode.py)."""
...
def _sample(logits: torch.Tensor, temperature: float, top_p: float):
    ...
    if temperature <= 0:  # greedy draws nothing from the generator
        idx = int(raw.argmax())
        return idx, float(raw[idx]), entropy
    probs = _dist(logits, temperature, top_p)
...
class Engine:
    ...
    def _setup_decode(self) -> None:
        """The resident decode arena (the graph decoder captures lazily, on CUDA)."""
    ...
    def _generate(self, messages, params: GenParams, emit, stop: threading.Event, context: dict) -> None:
        ...
        reason = "max_bytes"
        last_stats_bytes = 0
        script = list(params.script or [])  # forced ids (tests / replay): the eager path only
        calls_made = 0
        ...
        while not use_graph and len(generated) < params.max_bytes:
            ...
            if byte in STOP_IDS:
                ...
                break
            lg_next, routing, is_b = self.model.step(torch.tensor([[byte]], device=self.device), state)
    ...
    def _graph_decode(self, state, logits, params: GenParams, P: int, limit: int, n_done: int, record, stop: threading.Event):
        gd = self._graph_decoder()
